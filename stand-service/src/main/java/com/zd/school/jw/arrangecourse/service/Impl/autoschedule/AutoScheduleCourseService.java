...
public class AutoScheduleCourseService {
	...
	public  ClassWrap[] startScheduleCourse(AcsBaseinfo acsbaseinfo) throws Exception{
		...
		List<TeaTeacherbase> teachers= teaTeacherbaseService.queryByHql(queryTeacherHQL);
		List<JwTGradeclass> gclases =jwTGradeclassService.queryByHql(queryClassHQL);
		List<AcsCoursefeature> coursefeatures=coursefeatureService.queryAll();
	...
	private void classRelatCourse(ClassWrap cw,AcsBaseinfo acsbaseinfo,List<AcsCoursefeature> coursefeatures,List<TeacherWrap> teacherwps){
		...
		List<Object[]> bs=baseinfoService.queryEntityByHql(classCoursehql,cw.gclass.getUuid());
