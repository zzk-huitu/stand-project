...
Ext.define('Ext.ux.DateTimePicker', {
    ...
    renderTpl: [
        '<div id="{id}-innerEl" data-ref="innerEl" role="presentation">',
            ...
            '</table>',
            '<tpl if="showToday">',            
                '<div id="{id}-footerEl" role="presentation" style="background-color:#D9E5F3;border-top:0px solid #99BCE8;padding:2px 0px;">{%this.renderHour(values, out)%}{%this.renderMinute(values, out)%}{%this.renderSecond(values, out)%}<center>{%this.renderTodayBtn(values, out)%}</center></div>',
            '</tpl>',
        ...
        '</div>',
        {
            ...
            renderMinute: function(values, out) {
                out.push('<font  style="float : left;font-weight:bold;margin-top:5px">&nbsp:&nbsp&nbsp</font>');
